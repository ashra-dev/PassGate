...
try {
    ...
    refreshStallSessionFromDb($db);
    $stall_category = (string) ($_SESSION['stall_category'] ?? $stall_category);
...
$display_category = $stall_category !== '' ? $stall_category : 'general';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body class="pg-body pg-shell">
  ...
  <header class="pg-topbar">
    <div style="display:flex;align-items:center;gap:0.75rem;min-width:0;">
      ...
      <div style="min-width:0;">
        <div id="header-station-title" class="pg-station-name" style="text-transform:capitalize;"><?php echo htmlspecialchars($display_category); ?></div>
        <div style="margin-top:0.2rem;">
          <span id="header-status-chip" class="pg-status-chip is-ready">Ready</span>
        </div>
        <?php if ($stall_name !== '' && strtolower($stall_name) !== strtolower($display_category)): ?>
  ...
  <main class="pg-main">
    ...
    <div id="panel-scan">
    <div class="pg-panel" style="margin-bottom:0.85rem;" id="scan-category-panel">
      <p class="pg-section-title" style="margin:0 0 0.25rem;">Scanning category</p>
      <p id="scan-category-label" style="margin:0;font-weight:700;font-size:1rem;text-transform:capitalize;"><?php echo htmlspecialchars($display_category); ?></p>
      <p class="pg-faint" style="margin:0.35rem 0 0;font-size:0.75rem;">Benefits shown below are filtered to this category for each ticket.</p>
    </div>
      ...
      <div class="pg-details__body">
        <div id="unified-audit-panel">
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.75rem;font-size:0.8rem;">
            ...
            <div id="audit-category-row" style="grid-column:1 / -1;">
              <span class="pg-section-title" style="display:block;margin-bottom:0.25rem;">Category</span>
              <span id="audit-category" style="font-weight:600;text-transform:capitalize;"><?php echo htmlspecialchars($display_category); ?></span>
            </div>
          </div>
  ...
  <script>
    ...
    const STALL_CATEGORY = <?php echo json_encode($display_category); ?>;
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      if (SHOW_WELCOME) {
        showToast(`Welcome — ${formatCategoryLabel(STALL_CATEGORY)} station`, 'success');
      }
    });
    ...
    function populateBenefitDropdown(benefits, meta) {
      ...
      if (metaEl && meta) {
        const category = payload.stall_category || STALL_CATEGORY;
        const parts = [
    ...
    function renderStatusResult(d) {
      ...
      const benefitsHtml = (d.benefits || []).map(b => {
        ...
        const cat = b.category ? `<span class="pg-faint" style="font-size:0.65rem;text-transform:capitalize;margin-left:0.35rem;">(${escapeHtml(formatCategoryLabel(b.category))})</span>` : '';
        return `<div class="pg-benefit${full ? ' is-full' : ''}">
          <div style="display:flex;justify-content:space-between;gap:0.5rem;">
            <span style="font-weight:600;">${escapeHtml(b.name)}${cat}</span>
    ...
    function executeScanTransaction(ticketId) {
      ...
      .then(async response => {
        ...
        if (response.ok || data.status === 'already_scanned' || data.status === 'limit_reached') {
          ...
          const categoryLabel = formatCategoryLabel(data.stall_category || STALL_CATEGORY);
          showFlash(
            isGranted ? 'ok' : 'warn',
            isGranted ? 'GRANTED' : 'LIMIT REACHED',
            isGranted
              ? `Benefit approved (${categoryLabel}).`
              : 'Already claimed or max uses reached. Send to admin if needed.',
            cleanedId
          );
          ...
          if (auditCategory) {
            auditCategory.textContent = formatCategoryLabel(data.stall_category || STALL_CATEGORY);
          }
    ...
    function renderBenefitsUsagePanel(data) {
      ...
      const category = formatCategoryLabel(data.stall_category || STALL_CATEGORY);
      meta.textContent = [`Category: ${category}`, event, tier].filter(Boolean).join(' · ');
